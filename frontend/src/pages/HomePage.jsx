      ...
      <main className="flex-1 flex items-center justify-center py-16 px-4">
        ...
        <h2 className="text-4xl text-blue-700 mb-6">Pick Two Teams</h2>
          <div className="flex flex-row gap-4 justify-center flex-wrap">
            <select value={homeTeam} onChange={(e) => setHomeTeam(e.target.value)} className="w-64 px-4 py-2 border rounded-md">
              <option value="">Home Team</option>
              {nbaTeams.map(team => <option key={team} value={team}>{team}</option>)}
            </select>
            <select value={awayTeam} onChange={(e) => setAwayTeam(e.target.value)} className="w-64 px-4 py-2 border rounded-md">
